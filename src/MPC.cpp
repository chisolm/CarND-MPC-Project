...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int t = 0; t < N - 2; t++) {
      fg[0] += w_delta_diff * CppAD::pow(vars[delta_start + t + 1] -
          vars[delta_start + t], 2);
      fg[0] += w_a_diff * CppAD::pow(vars[a_start + t + 1] -
          vars[a_start + t], 2);
    }
    ...
    for (int t = 0; t < N - 1; t++) {
      ...
      // Contraints for computation of t+1 state from t state
      fg[2 + x_start + t] = x1 - (x0 + v0 * CppAD::cos(psi0) * dt);
      fg[2 + y_start + t] = y1 - (y0 + v0 * CppAD::sin(psi0) * dt);
      fg[2 + psi_start + t] = psi1 - (psi0 + v0 * delta0 / Lf * dt);
      fg[2 + v_start + t] = v1 - (v0 + a0 * dt);
      fg[2 + cte_start + t] = cte1 - ((f0 - y0) + v0 * CppAD::sin(epsi0) * dt);
      fg[2 + epsi_start + t] = epsi1 - ((psi0 - psides0) + v0 *
          delta0 / Lf * dt);
    }
  }
};
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // 6 input variables(N states), 2 actuator variables(N - 1) states
